...
interface BRAINToken is IERC20 {
...
contract MasterChefBrain is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_pid == 1) {
            stakedJoe = stakedJoe.add(_amount);
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_pid == 1) {
            stakedJoe = stakedJoe.sub(_amount);
        }
    }
    
    function compoundToSas(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        updatePool(_pid);
            
        UserInfo storage sasPoolUser = userInfo[1][msg.sender];
        
        sasPoolUser.amount = sasPoolUser.amount.add(user.amount.mul(pool.accJoePerShare).div(1e12).sub(
            user.rewardDebt));
        
        stakedJoe = stakedJoe.add(user.amount.mul(pool.accJoePerShare).div(1e12).sub(
            user.rewardDebt));
