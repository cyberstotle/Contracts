...
contract BRAIN is Ownable, ERC20, BRAINToken {
    ...
    constructor( string memory name, string memory symbol, uint256 _Max_t, address payable _marketingWallet, address payable _treasuryWallet, address _burnWallet, uint256 _tax) ERC20(name, symbol){
    ...
    function _transfer(
        ...
        uint256 amount
            ...
            if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && tax_on) {
                ...
                swapTokensForEth(amount.mul(tax).div(100));
                uint256 senderBalance = _balances[sender];
                unchecked {
                    _balances[sender] = senderBalance - amount;
                }
